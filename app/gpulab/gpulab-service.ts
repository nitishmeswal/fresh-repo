...
export class GPULabClient {
    private static instance: GPULabClient | null = null;
    private static lock = false;
    private readonly baseUrl = 'https://api.gpulab.ai';
    ...
    public static getInstance(): GPULabClient {
        if (!GPULabClient.instance) {
            if (GPULabClient.lock) {
                throw new Error('GPULabClient is being initialized');
            }
            GPULabClient.lock = true;
            GPULabClient.instance = new GPULabClient();
            GPULabClient.lock = false;
        }
